...
void PerformTransactionsOption(enATMMenueOptions);
...
{
	// في المستقبل عندما نعود لهذا المشروع سنعدل على هذه الدالة ونعتمد الجنرال يوزر
	// وأيضا نعتمد الإينم بريمشنز.
...
void NormalDeposite()
{
	...
	if (ReadAnswer(Message))
	{
		GeneralClient.AccountBalance += Amount;
		cout << "Done Successfully, New balance is : " << GeneralClient.AccountBalance << endl;
...
void PerformQuickWithdrawOptions(vector <stClient>& vClients)
{
	...
	if (Number > 0 && Number < 10)
	{
		...
		if (ReadAnswer(Message))
		{
			DepositBalanceToClientByAccountNumber(GeneralClient.AccountNumber, Amount * -1, vClients);
		}
	}
}
...
void QuickWithdraw(string AccountNumber)
{
	...
	cout << "Your balance is " << GeneralClient.AccountBalance << endl;
...
void PerformATMOption(enATMMenueOptions TransactionOption)
{
	vector <stClient> vClients = LoadClientsDataFromFileToV(ClientsFileName);
...
{
	ShowATMMenue();
	PerformATMOption(ReadATMMenueOption());
}
...
bool CheckLogingATM(string AccountNumber, string PinCode)
{
	...
	if (IsFoundAccount(vClients, Client, Client.AccountNumber))
	{
